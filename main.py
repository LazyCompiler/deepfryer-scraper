...
def get_urls(amount_of_messages: int):
    ...
    if len(messages) > 0:
        ...
        parsed_json_messages = [json.loads(parsed_json_body['Message']) for parsed_json_body in parsed_json_bodies]
...
if __name__ == '__main__':
    ...
    scrap_urls = get_urls(args.amount_of_messages) if len(args.debug_urls) == 0 else args.debug_urls
    ...
    print(f'Average time per item: {(ending_time - starting_time) / len(scrap_urls)} seconds')
    print('Done')
