...
class PricescraperPipeline:
    ...
    def process_item(self, item, spider):
        ...
        if (
            ...
            and "eilat_price" not in item
            and "origin_price" not in item
        ):
    ...
    def close_spider(self, spider):
            ...
            records.append(
                {
                    ...
                    "MeasureValue": str(
                        item["current_price"]
                        if not math.isnan(item["current_price"])
                        else -1
                    ),
                    "MeasureValueType": "DOUBLE",
                    "Time": str(item["time"]),
                    "Dimensions": [
                        {"Name": "item_id", "Value": str(item["id"])},
                        {"Name": "item_name", "Value": str(item["name"])},
                        {
                            "Name": "origin_price",
                            "Value": str(item["origin_price"])
                            if "origin_price" in item
                            else None,
                        },
                        {"Name": "eilat_price", "Value": str(item["eilat_price"])},
                        {"Name": "current_price", "Value": str(item["current_price"])},
                    ],
                }
            )
        ...
        for index, records_chunk in enumerate(records_chunks):
            ...
            result = self.client.write_records(
                DatabaseName="deepfryer", TableName="prices", Records=records_chunk
            )
